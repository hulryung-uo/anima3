...
def test_train_verbs_largest_gap_first_and_rate_limited():
    ...
    w = FakeBody(); w.add_pack_item(BANDAGE_GRAPHIC, 5); w.add_pack_item(0x0E86)   # a pickaxe: gear for ArmsLore
    ...
    assert ids == ["train:ArmsLore", "train:Hiding"]      # Meditation skipped: mana is full
    mem["train_next"] = 200
...
def test_train_procedure_uses_skill_then_targets_when_needed():
    ...
    gen2 = _train_proc(4, "gear", 0x77)(o, {"tick": 5}); assert next(gen2)["type"] == "UseSkill"
