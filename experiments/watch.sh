...
errs(){ cat $FILES 2>/dev/null | grep -cE "never started|Traceback|crashed|MISSING|void:|preflight"; }
...
while true; do
  n=$(tot); e=$(errs); f=$(fin)
  [ "$e" -gt "$seen_err" ] && { cat $FILES | grep -E "never started|Traceback|crashed|MISSING|void:|preflight" | tail -n $((e-seen_err)); seen_err=$e; }
